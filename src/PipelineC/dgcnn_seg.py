...
class DGCNN_seg(nn.Module):
    def __init__(self, num_classes=2, k=20, emb_dims=2048, dropout=0.05, in_channels=3):
        super().__init__()
        self.k = k
        self.conv1 = nn.Sequential(nn.Conv2d(2*in_channels, 64, 1), 
                                   nn.BatchNorm2d(64), 
                                   nn.LeakyReLU(0.2))  
        self.conv2 = nn.Sequential(nn.Conv2d(128, 64, 1), 
                                   nn.BatchNorm2d(64), 
                                   nn.LeakyReLU(0.2))
        self.conv3 = nn.Sequential(nn.Conv2d(128, 128, 1), 
                                   nn.BatchNorm2d(128), 
                                   nn.LeakyReLU(0.2))
        self.conv4 = nn.Sequential(nn.Conv2d(256, 256, 1), 
                                   nn.BatchNorm2d(256), 
                                   nn.LeakyReLU(0.2))
        self.conv5 = nn.Sequential(nn.Conv1d(512, emb_dims, 1), 
    ...
    def forward(self, x):  # x: B x in_channels x N
